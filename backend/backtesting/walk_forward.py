...
class WalkForwardValidator:
    ...
    def validate(self, df: pd.DataFrame, symbol: str, train_days: int = 30, test_days: int = 10, step_days: int = 10) -> Dict[str, Any]:
        """
        ...
        while current_start + datetime.timedelta(days=train_days) < end_date:
            ...
            if len(test_df) > 0:
                ...
                engine = BacktestEngine(strategy_instance, initial_capital=self.initial_capital)
                ...
                warmup_start = train_end - datetime.timedelta(days=10) # 10 days warm-up
                ...
                test_trades = [t for t in engine.broker.trades if t['entry_time'] >= train_end]
                all_test_trades.extend(test_trades)

                window_metrics = MetricsEvaluator.evaluate(test_trades, self.initial_capital)
                results.append({
        ...
        # Overall OOS Metrics
        overall_metrics = MetricsEvaluator.evaluate(all_test_trades, self.initial_capital)
