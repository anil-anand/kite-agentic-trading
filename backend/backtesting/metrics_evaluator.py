...
class MetricsEvaluator:
    @staticmethod
    def evaluate(trades: List[Dict[str, Any]], initial_capital: float) -> Dict[str, Any]:
        if not trades:
            return {}
        ...
        df['holding_time'] = (df['exit_time'] - df['entry_time']).dt.total_seconds() / 60.0 # in minutes
        ...
        profit_factor = (wins['net_pnl'].sum() / abs(losses['net_pnl'].sum())) if abs(losses['net_pnl'].sum()) > 0 else float('inf')
        ...
        for idx, row in df.iterrows():
            ...
            if sl and row['entry_price'] != sl:
                ...
                df.at[idx, 'empirical_r'] = row['net_pnl'] / (risk_per_share * row['quantity'])
        ...
        avg_mfe = df.get('mfe', pd.Series(dtype=float)).mean() if 'mfe' in df.columns else 0.0
        avg_mae = df.get('mae', pd.Series(dtype=float)).mean() if 'mae' in df.columns else 0.0
        ...
        return {
            ...
            "cost_contribution_pct": round((df['total_fees'].sum() / abs(df['gross_pnl'].sum())) * 100, 2) if df['gross_pnl'].sum() != 0 else 0.0,
