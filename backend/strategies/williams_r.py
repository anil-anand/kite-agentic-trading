...
class WilliamsRStrategy(BaseStrategy):
    ...
    def calculate_signals(self, df, symbol: str):
        ...
        if prev['wr'] < -80 and last['wr'] >= -80:
            ...
            signals.append(self.format_signal(symbol, "BUY", 70, last['close'], sl, target, round(abs(target-entry)/abs(entry-sl), 2) if entry != sl else 0, "Williams %R crossed above -80", {}))
        elif prev['wr'] > -20 and last['wr'] <= -20:
            sl = self.calculate_stop_loss(last['close'], "SELL", 0.6)
            target = self.calculate_target(last['close'], sl, 1.2)
            signals.append(self.format_signal(symbol, "SELL", 70, last['close'], sl, target, round(abs(target-entry)/abs(entry-sl), 2) if entry != sl else 0, "Williams %R crossed below -20", {}))
        return signals
