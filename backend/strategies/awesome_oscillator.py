...
class AwesomeOscillatorStrategy(BaseStrategy):
    ...
    def calculate_signals(self, df, symbol: str):
        ...
        if prev['ao'] < 0 and last['ao'] >= 0:
            ...
            signals.append(self.format_signal(symbol, "BUY", 74, last['close'], sl, target, round(abs(target-entry)/abs(entry-sl), 2) if entry != sl else 0, "AO crossed above zero line", {}))
        elif prev['ao'] > 0 and last['ao'] <= 0:
            sl = self.calculate_stop_loss(last['close'], "SELL", 0.6)
            target = self.calculate_target(last['close'], sl, 1.5)
            signals.append(self.format_signal(symbol, "SELL", 74, last['close'], sl, target, round(abs(target-entry)/abs(entry-sl), 2) if entry != sl else 0, "AO crossed below zero line", {}))
        return signals
