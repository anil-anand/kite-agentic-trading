...
class TSICrossStrategy(BaseStrategy):
    ...
    def calculate_signals(self, df, symbol: str):
        ...
        if prev['tsi'] < 0 and last['tsi'] >= 0:
            ...
            signals.append(self.format_signal(symbol, "BUY", 76, last['close'], sl, target, round(abs(target-last['close'])/abs(last['close']-sl), 2) if last['close'] != sl else 0, "TSI crossed above zero line", {}))
        elif prev['tsi'] > 0 and last['tsi'] <= 0:
            sl = self.calculate_stop_loss(last['close'], "SELL", 0.8)
            target = self.calculate_target(last['close'], sl, 1.5)
            signals.append(self.format_signal(symbol, "SELL", 76, last['close'], sl, target, round(abs(target-last['close'])/abs(last['close']-sl), 2) if last['close'] != sl else 0, "TSI crossed below zero line", {}))
        return signals
