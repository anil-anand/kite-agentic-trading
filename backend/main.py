...
def handle_request(req):
    ...
    def error(code, message, data=None):
        return {"jsonrpc": "2.0", "error": {"code": code, "message": message, "data": data}, "id": req_id}
    ...
    try:
        if method == "login":
            ...
            config_manager.save_credentials(api_key, api_secret, session["access_token"])
            ...
            total_trades = risk_manager.win_count + risk_manager.loss_count
            win_rate = (risk_manager.win_count / total_trades * 100) if total_trades > 0 else 0
            
            summary = {
                "totalPnl": risk_manager.daily_pnl,
                "tradesToday": total_trades,
...
def main():
    for line in sys.stdin:
        ...
        try:
            ...
            res = {"jsonrpc": "2.0", "error": {"code": -32700, "message": "Parse error"}, "id": None}
